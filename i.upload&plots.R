...
#write.table(uploadTb3,"20190221 upload.txt",row.names=FALSE,quote=FALSE)



#DimentionTable<-data.frame('Table'=c('All_Out','All_Make'),
#                           'NumSched_In'=c(dim(comb_Out)[1],dim(make_output)[1]),
#                           'NumSched_Out'=c(dim(MoMSchedules)[1]/10,dim(join_MakeAdj)[1]/10))



#print(if(DimentionTable[2,3]+1 == DimRegular[1]){paste('Yes, regular schedules are ', DimRegular[1])} else {'No'})
#print(if(DimentionTable[1,3]+DimentionTable[3,3] == DimMake[1]){paste('Yes, the Make Adjustment schedules are', DimMake[1])} else {'No'})
...
for (j in 1:N_plot){
  ...
  list<-listingCrane %>% filter(Schedule==SchedFullList_plot[j,1])
  ...
  LastMonth_Sched<-subset(Last_Sched,Last_Sched$Schedule==SchedFullList_plot[j,1])
  ...
  draw_lastM_Auc<-xyplot(CurrentFlv ~ Age, LastMonth_Sched,ylim=yaxis,xlim=xaxis,type=c('p','l'),lty=3,col='dodgerblue3',ylab = yaxis_name,lwd=3,pch=4,cex=1.5)
  ...
  draw_Sched_Ret<-xyplot(limit_fmv ~ Age, NewMonth_Sched,ylim=yaxis,xlim=xaxis,scales = list(y=list(tick.number=10),x=list(tick.number=16))
                         ...
                         ylab = yaxis_name,main=list(label=paste(SchedFullList_plot[j,1],' - ' ,publishDate),font=2,cex=2))
  draw_lastM_Ret<-xyplot(CurrentFmv ~ Age, LastMonth_Sched,ylim=yaxis,xlim=xaxis,type=c('p','l'),lty=3,col='firebrick2',ylab = yaxis_name,lwd=3,pch=4,cex=1.5)
